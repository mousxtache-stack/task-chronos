...
import { loadStripe, Stripe } from '@stripe/stripe-js'; // Importer loadStripe et le type Stripe

// Charger Stripe.js en dehors du rendu du composant
...
if (STRIPE_PUBLISHABLE_KEY) {
  ...
  // Gérer le cas où la clé n'est pas là, peut-être désactiver le bouton de paiement
}
...
const PremiumPage = () => {
  ...
  useEffect(() => {
    ...
    if (query.get('payment_success') && query.get('stripe_session_id')) {
      ...
      // Le webhook Stripe devrait mettre à jour la base de données.
      // On recharge pour que Index.tsx récupère le nouveau statut.
      setTimeout(() => {
          navigate('/');
  ...
  const handleUpgradeToPremium = async () => {
    ...
    try {
      ...
      if (stripeError) {
        ...
        setIsLoading(false); // Important de remettre isLoading à false si redirectToCheckout échoue
      }
      // Si redirectToCheckout réussit, l'utilisateur est redirigé et ne revient pas ici immédiatement.
      // setIsLoading(false) sera géré par la redirection ou l'échec.
  ...
  return (
    <>
      <Navbar />
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-indigo-900 text-white py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <section className="grid md:grid-cols-3 gap-8 mb-16">
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-xl shadow-lg hover:shadow-purple-500/50 transition-shadow">
          ...
          <section className="text-center bg-white/5 p-8 rounded-xl shadow-2xl">
            ...
            <Button 
              size="lg" 
              className="bg-yellow-400 text-indigo-700 hover:bg-yellow-500 font-bold text-lg px-10 py-4 shadow-md hover:shadow-lg transition-all"
          ...
          <footer className="text-center mt-16">
            <Link to="/" className="text-purple-300 hover:text-yellow-400 transition-colors">
